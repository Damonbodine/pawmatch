# Brownfield Agent — pawmatch-v3

**Run ID:** n9726z8rgmn4087tc2gjnyz2dx83sf3k
**Project ID:** jd76qtmqr6n6q7qsw8qnkktg0x83rnh2
**Repo:** /Users/damonbodine/Factory512/builds/pawmatch-v3-20260324-202428/pawmatch-v3
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Adoption Match Scorer — on application detail page, analyze adopter lifestyle vs animal temperament/needs/medical conditions. Return match score 1-100 with compatibility breakdown. Show as card with score gauge + factor analysis. 2) Animal Bio Writer — on animal detail page, button to generate compelling adoption listing from structured data (species, breed, age, temperament tags, medical notes). Offer tone options: heartwarming, playful, dignified. 3) Behavioral Assessment Summarizer — on animal profile, synthesize behavioral observations into standardized profile: sociability, energy level, training needs, ideal home type. 4) Adoption Follow-Up Generator — after adoption, generate personalized 2-week check-in email based on animal's known needs and adopter's situation. Each feature needs: Convex action, React component with loading states, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/pawmatch-v3-20260324-202428/pawmatch-v3. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n9726z8rgmn4087tc2gjnyz2dx83sf3k"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n9726z8rgmn4087tc2gjnyz2dx83sf3k"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n9726z8rgmn4087tc2gjnyz2dx83sf3k","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n9726z8rgmn4087tc2gjnyz2dx83sf3k","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n9726z8rgmn4087tc2gjnyz2dx83sf3k","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/pawmatch-v3-20260324-202428/pawmatch-v3`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Adoption Match Scorer — on application detail page, analyze adopter lifestyle vs animal temperament/needs/medical conditions. Return match score 1-100 with compatibility breakdown. Show as card with score gauge + factor analysis. 2) Animal Bio Writer — on animal detail page, button to generate compelling adoption listing from structured data (species, breed, age, temperament tags, medical notes). Offer tone options: heartwarming, playful, dignified. 3) Behavioral Assessment Summarizer — on animal profile, synthesize behavioral observations into standardized profile: sociability, energy level, training needs, ideal home type. 4) Adoption Follow-Up Generator — after adoption, generate personalized 2-week check-in email based on animal's known needs and adopter's situation. Each feature needs: Convex action, React component with loading states, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n9726z8rgmn4087tc2gjnyz2dx83sf3k",
  "projectName": "pawmatch-v3",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Adoption Match Scorer — on application detail page, analyze adopter lifestyle vs animal temperament/needs/medical conditions. Return match score 1-100 with compatibility breakdown. Show as card with score gauge + factor analysis. 2) Animal Bio Writer — on animal detail page, button to generate compelling adoption listing from structured data (species, breed, age, temperament tags, medical notes). Offer tone options: heartwarming, playful, dignified. 3) Behavioral Assessment Summarizer — on animal profile, synthesize behavioral observations into standardized profile: sociability, energy level, training needs, ideal home type. 4) Adoption Follow-Up Generator — after adoption, generate personalized 2-week check-in email based on animal's known needs and adopter's situation. Each feature needs: Convex action, React component with loading states, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/pawmatch-v3-20260324-202428/pawmatch-v3`
